...
これは武蔵野アドベントカレンダー14日目の記事です。

私は武蔵野とか横須賀とかが関係する事情でOpenFlow1.3で必要十分、DPDK万歳派閥に属していたため、XDPとP4は知識でしか知らなかった。
そこで、今回の機会使って、XDPとP4を触ってみることにする。
ググるとp4c-xdpという一石二鳥なレポジトリがあったので今回はこれを触ってみた内容を紹介する。
p4cの公式にもp4c-ebpfとかいうディレクトリがあり、そっちのほうが更新が進んでいるがどうやらtc向けのebpfを吐くようだ。勉強不足であまり理解はしていない。

## 今回の目標

Turtrial 動かしましたブログを量産したくはないので、すこし応用が聞きそうなことをする。
具体的には、Vxlanの中身のsrcIPのアドレスによってパケットフィルタを行う。
下のVM2のens7にXDPをアタッチし、VM1のns0からのパケットのみフィルタする。
...
- トンネルの中を覗き込むという変なことできるXDP強い。さらにXDPのあとにLinuxのNetwork Stackがなんとかしてくれるという安心感は大きい。
- オレオレヘッダでもなんでも簡単にパーサ、マッチアクション、デパーサ書けるP4強い。
- パケットパーサ部分が自動生成されるのは強いので、ここの部分だけ使うのはありかもしれない。　
- P4の簡単さとXDPの強さを一緒に使えるp4c-xdpには夢がある。ただebpfっていう限られた環境でp4使い倒すのは難しい。
...
一般に、P4とはペルソナ４の略であるが、ネットワーク業界におけるP4とはパケットをどういうヘッダのパケットをどう転送するかみたいなことが自由に記述できる言語である。
...
ただ、何ができるかという点については、P4でXDPを動かすという前提から、p4c-xdpが動く範囲はp4で記述できる範囲でなおかつXDPで動作可能な範囲に限られる。
...
p4c, p4c-xdpのインストール。p4c-xdpはp4cのextensionとしてはいる。p4cのCommitはv1.0.0-rc4っぽいやつにした。
...
とりあえず今回は `xdp1.p4` を参考にした。
これにVxLANとInnerのパーサを追加して、もともとether_typeが0x0800でなければ落とすというものだったものを、VxLAN内部のsrcAddrが192.160.0.10(0xC0A8000A))だったら落とすという処理に変更した。
コードは以下の通り。
...
このままではエラーがでてコンパイルできない。なぜなら大きすぎるから。
生成されたソースコードを読んでみると、あきらかにdeparser部分が重いことがわかる。
...
3232235530は192.168.0.10なので正しく動きそうに見えるが、これだとARPなど、VxLANの内部にIPがなかった場合の考慮が足りない(if文でhd.inner_ipv4.srcAddrが未定義のまま比較されてしまう)ので以下のように修正する。
